...
rct_image_tools::CircleDetectorParams loadTargetFinderParams(const YAML::Node target_params_yaml)
{
  ...
  loadOptionalField(
    target_params_yaml, "circleInclusionRadius",
    target_finder_params.circleInclusionRadius);
  loadOptionalField(target_params_yaml, "maxRadiusDiff", target_finder_params.maxRadiusDiff);
  loadOptionalField(
    target_params_yaml, "maxAverageEllipseError",
    target_finder_params.maxAverageEllipseError);
  ...
  loadOptionalField(
    target_params_yaml, "filterByCircularity",
    target_finder_params.filterByCircularity);
  ...
  loadOptionalField(
    target_params_yaml, "filterByConvexity",
    target_finder_params.filterByConvexity);
...
static const std::string DEBUG_FILE_PARAMETER = "debug_filepath";
...
public:
  ...
  : Node("camera_on_wrist_server")
  {
    ...
    run_wrist_cal_srv_ =
      ...
      "run_wrist_cal",
      std::bind(&CameraOnWristServer::runWristCalCB, this, _1, _2));

    declare_parameter<std::string>(CONFIG_FILE_PARAMETER, "");
    declare_parameter<std::string>(DATA_FILE_PARAMETER, "");
    declare_parameter<std::string>(DEBUG_FILE_PARAMETER, "/tmp/calibration_debug");
  }
...
private:
  ...
  {
    std::string config_filepath, data_filepath, debug_filepath;
    try {
      ...
      if (!get_parameter(DEBUG_FILE_PARAMETER, debug_filepath)) {
        throw std::runtime_error("Failed to get parameter '" + DEBUG_FILE_PARAMETER + "'");
      }
    ...
    auto check_fields = [this, config_yaml, response](const std::string field)
      {
        if (!config_yaml[field]) {
          ...
          RCLCPP_ERROR_STREAM(
            get_logger(),
            "Run Wrist cal service failed because yaml field " << field << " was not found");
          return false;
        }
        return true;
      };
    ...
    rct_image_tools::CircleDetectorParams target_finder_params =
      loadTargetFinderParams(config_yaml["target_finder_params"]);
    ...
    if (!maybe_data_set) {
      ...
      RCLCPP_ERROR_STREAM(
        get_logger(),
        "Run Wrist cal service failed: '" << "Failed to parse data set from path = " << data_filepath <<
          "'");
      return;
    }
    ...
    rct_image_tools::CircleDetector circle_detector(target_finder_params);
    mkdir(debug_filepath.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);

    for (std::size_t i = 0; i < image_set.size(); ++i) {
      auto drawn_image = circle_detector.drawDetectedCircles(image_set[i]);
      cv::imwrite(debug_filepath + "/image" + std::to_string(i) + ".png", drawn_image);
    ...
    printOptResults(
      opt_result.converged, opt_result.initial_cost_per_obs,
      opt_result.final_cost_per_obs);
    printNewLine();
